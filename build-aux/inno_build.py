...
for loader in glob.glob(
    f"{build_environment_path}/lib/gdk-pixbuf-2.0/2.10.0/loaders/*.dll"
):
    run_command(
...
shutil.copy(f"{build_root}/data/{app_id}.gschema.xml", gschemas_dir)

# generate `gschemas.compiled` in the same directory
run_command(f"glib-compile-schemas {gschemas_dir}", "Compiling schemas failed")
...
# app locale
app_mo_dir = os.path.join(build_root, "po")
...
for file in os.listdir(app_mo_dir):
    ...
    current_system_locale_dir = os.path.join(
        build_environment_path, "share/locale", current_lang, "LC_MESSAGES"
    )
...
run_command(f"iscc {inno_script}", "Running ISCC failed")
